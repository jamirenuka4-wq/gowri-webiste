...
function getDb() {
  ...
  if (isVercel) {
    dbPath = '/tmp/gowri_pickles.db';
    // On cold start: copy seed.db from project root to /tmp if it exists
    // If seed.db is missing, a fresh DB will be created and seeded below
    if (!fs.existsSync(dbPath)) {
      const seedPath = path.join(process.cwd(), 'seed.db');
      if (fs.existsSync(seedPath)) {
        fs.copyFileSync(seedPath, dbPath);
        console.log('✅ DB: Copied seed.db to /tmp');
      } else {
        console.log('ℹ️ DB: No seed.db found — creating fresh database in /tmp');
      }
    }
  } else {
    dbPath = path.join(process.cwd(), 'gowri_pickles.db');
  }
...
function seedData(db) {
  ...
  if (productCount === 0) {
    const ins = db.prepare(`INSERT INTO products (name,description,price,original_price,category,image,weight,spice_level,ingredients,shelf_life,featured,rating,reviews_count) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`);
    const products = [
      ...
      ['Tomato Pickle','Tangy sun-ripened tomato pickle with a perfect blend of South Indian spices.',109,139,'Special','/images/mango_pickle.png','250g','Medium','Tomato, Red Chilli, Sesame Oil, Mustard Seeds, Curry Leaves, Salt, Turmeric','6 months',0,4.5,63],
      ...
      ['Lemon Ginger Pickle','Zesty combination of fresh lemon and ginger pickled with rock salt and spices. Aids digestion.',139,179,'Lemon','/images/lemon_pickle.png','250g','Mild','Lemon, Ginger, Rock Salt, Red Chilli, Turmeric, Mustard Seeds','18 months',0,4.6,47],
    ];
  ...
  if (reviewCount === 0) {
    const insR = db.prepare('INSERT INTO reviews (product_id, customer_name, rating, comment) VALUES (?,?,?,?)');
    const tx = db.transaction((rows) => { for (const r of rows) insR.run(...r); });
    tx([
